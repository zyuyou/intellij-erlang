...
final class Rebar3SelectImportedOtpAppsStep extends SelectImportedProjectsStep<Rebar3ImportedOtpApp> {
  private final Set<String> myDuplicateModuleNames = new HashSet<String>();

  public Rebar3SelectImportedOtpAppsStep(@NotNull WizardContext context) {
    super(context);
    fileChooser.addElementsMarkListener(new ElementsChooser.ElementsMarkListener<Rebar3ImportedOtpApp>() {
      public void elementMarkChanged(@NotNull Rebar3ImportedOtpApp importedOtpApp, boolean isMarked) {
        evalDuplicates();
        fileChooser.repaint();
      }
    });
  ...
  public void autoResolveConflicts() {
    ...
    for (Rebar3ImportedOtpApp selectedOtpApp : fileChooser.getMarkedElements()) {
  ...
  @Override
  protected String getElementText(@NotNull Rebar3ImportedOtpApp importedOtpApp) {
    return importedOtpApp.toString();
  }

  @Nullable
  @Override
  protected Icon getElementIcon(@NotNull Rebar3ImportedOtpApp importedOtpApp) {
  ...
  private void evalDuplicates() {
    List<Rebar3ImportedOtpApp> selectedOtpApps = fileChooser.getMarkedElements();
    Set<String> contains = new HashSet<String>(selectedOtpApps.size());
    myDuplicateModuleNames.clear();
    for (Rebar3ImportedOtpApp importedOtpApp : selectedOtpApps) {
